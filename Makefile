...
# xf
KERNEL_XF_ASYM_ATTEST_NAME = XF_ASYMMETRIC_ATTEST
...
# xf
KERNEL_XF_ASYM_ATTEST_OBJ := $(BUILD_DIR)/$(KERNEL_XF_ASYM_ATTEST_NAME).xo
KERNEL_XF_ASYM_ATTEST_XCLBIN = $(BUILD_DIR)/$(KERNEL_XF_ASYM_ATTEST_NAME).xclbin
...
# xf
KERNEL_XF_ASYM_SRC_DIR := $(SRC_DIR)/xf-kernel/asym
...
# for XF Kernel
KERNEL_XF_ASYM_L1_DIR := $(KERNEL_ASYM_DEPS_DIR)/L1
...
# xf
KERNEL_XF_ASYM_SRCS := $(shell find $(KERNEL_XF_ASYM_SRC_DIR) -name '*.cpp' -or -name '*.c')

# xf includes
KERNEL_XF_INC_DIRS := $(shell find $(KERNEL_XF_ASYM_SRC_DIR) $(KERNEL_XF_ASYM_L1_DIR) -type d)
KERNEL_XF_INC_FLAGS := $(addprefix -I,$(KERNEL_XF_INC_DIRS))
...
# xf
HOST_EXEC_KERNEL_XF_ASYM_ATTEST_ARGS := -k $(KERNEL_XF_ASYM_ATTEST_NAME) -x $(KERNEL_XF_ASYM_ATTEST_XCLBIN) -n $(NUM_BENCHMARK_ITERATIONS)
...
# xf
kernel-xf-asym-attest: $(KERNEL_XF_ASYM_SRCS)
	$(VC)  --kernel $(KERNEL_XF_ASYM_ATTEST_NAME) --compile $(KERNEL_VC_FLAGS) $(KERNEL_XF_INC_FLAGS) $(KERNEL_XF_ASYM_SRCS) -o $(KERNEL_XF_ASYM_ATTEST_OBJ)
...
# xf
link-xf-asym-attest:
	$(VC) --link $(KERNEL_VC_FLAGS) $(KERNEL_XF_ASYM_ATTEST_OBJ) -o $(KERNEL_XF_ASYM_ATTEST_XCLBIN)
...
run-xf-asym-attest-benchmark:
	test -e $(HOST_TARGET)
	# BENCHMARK ATTEST
ifeq ($(COMPILE_TARGET), hw)
	$(HOST_TARGET) $(HOST_EXEC_KERNEL_XF_ASYM_ATTEST_ARGS)
else
	XCL_EMULATION_MODE=$(COMPILE_TARGET) $(HOST_TARGET) $(HOST_EXEC_KERNEL_XF_ASYM_ATTEST_ARGS)
endif
