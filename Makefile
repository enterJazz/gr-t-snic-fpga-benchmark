...
HOST_EXEC_VERIFY_VERIFY_ARGS := -k $(KERNEL_VERIFY_NAME) -x $(KERNEL_VERIFY_XCLBIN) -n $(NUM_BENCHMARK_ITERATIONS)
...
run-verify-benchmark:
	test -e $(HOST_TARGET)
	# RUN ATTEST
	XCL_EMULATION_MODE=$(COMPILE_TARGET) $(HOST_TARGET) $(HOST_EXEC_VERIFY_VERIFY_ARGS)
