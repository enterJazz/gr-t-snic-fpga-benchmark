...
#pragma HLS INTERFACE m_axi port = out_verify_result bundle = aximm1
  ...
  std::memcpy(&signed_message[sign_input_len], in_msg_attestation,
              attestation_len);

  int result = 1;
#if 0
      crypto_sign_open(sign_input_array, (long long unsigned *)sign_input_len,
                       signed_message, signed_message_len, pubkey);
#endif
