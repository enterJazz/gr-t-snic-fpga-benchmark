...
int main(int argc, char **argv) {
  ...
  if (input.cmdOptionExists(ParseOpts::help) || argc == 1) {
    std::cout << binary_name << " " << ParseOpts::getOption(ParseOpts::kernel)
  ...
  const std::string kernel_xlcbin_path_string{
      input.getCmdOption(ParseOpts::kernel_xlcbin)};
  ...
  benchmark::benchmark_kernel(kernel_enum, kernel_xlcbin_path, num_iterations);
