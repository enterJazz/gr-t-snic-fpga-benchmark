...
void benchmark_verify_kernel(
    std::chrono::microseconds &result, xrt::device device, xrt::kernel in_krnl,
    ...
    uint8_t *preset_msg_hash, uint8_t *preset_msg_attestation,
    uint8_t *preset_pubkey, uint8_t *attestation_result) {
  uint8_t output_attestation_size{(krnl_type == kernel::symmetric_verify)
  ...
  auto bo_in_attestation = xrt::bo(device, output_attestation_size,
                                   in_krnl.group_id(device_input_group_id));
  auto bo_in_pubkey = xrt::bo(device, eddsa_pubkey_size,
                              in_krnl.group_id(device_input_group_id));
  // output buffer
  ...
  utils::copy_else_fill(bo_in_msg_hash_map, input_msg_hash_size,
                        preset_msg_hash);
  utils::copy_else_fill(bo_in_attestation_map, output_attestation_size,
                        preset_msg_attestation);
  ...
  auto run{
      (krnl_type == kernel::symmetric_verify)
          ? in_krnl(bo_in_msg_hash, bo_in_attestation, bo_out)
          : in_krnl(bo_in_msg_hash, bo_in_attestation, bo_in_pubkey, bo_out)};
