...
namespace benchmark::verify
{
    ...
    )
    {
        ...
        auto bo_in = xrt::bo(
                device,
                (input_msg_hash_size + output_attestation_hash_size),
        ...
        std::fill(
            bo_in_map,
            bo_in_map + (input_msg_hash_size + output_attestation_hash_size),
            0
        );

        std::fill(bo_out_map, bo_out_map + output_attestation_hash_size, 0);

        utils::populate_input_data(bo_in_map, (input_msg_hash_size + output_attestation_hash_size));
