...
SCENARIO("the asymmetric attest kernel computes valid attestations") {
  GIVEN("the kernel, set to execute in sw_emu") {
    ...
    std::filesystem::path kernel_xlcbin_path{
        tu::get_env_var(tu::kernel_asym_attest_path_env_var)};
    ...
    WHEN("we execute the kernel with pre-set inputs") {
      ...
      attest::benchmark_attest_kernel(
          ...
          preset_msg_hash, attestation_result);

      THEN("we expect the generated signature to match the pre-computed "
           "signature") {
        REQUIRE(std::equal(tu::expected_signature,
                           tu::expected_signature + bm::eddsa_signature_size,
                           attestation_result));
      }
    }
  }
