...
SCENARIO("the asymmetric verify kernel distinguised in-/valid attestations") {
  GIVEN("the kernel, set to execute in sw_emu") {
    REQUIRE(tu::get_env_var(tu::xcl_emulation_mode_env_var) ==
            tu::xcl_sw_emu_mode);

    std::filesystem::path kernel_xlcbin_path{
        tu::get_env_var(tu::kernel_asym_verify_path_env_var)};
    ...
    auto krnl =
        xrt::kernel(device, uuid, kernel::kernelToString(asym_kernel_type),
    ...
    WHEN("we execute the kernel with pre-set inputs") {
      ...
      std::memcpy(preset_attestation_result, tu::expected_signature,
                  bm::eddsa_signature_size);
      ...
      verify::benchmark_verify_kernel(
          ...
          preset_msg_hash, preset_attestation_result, preset_pubkey,
          attestation_result);

      THEN("we expect the generated signature to match the pre-computed "
           "signature") {
